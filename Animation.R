...
p1 <- ggplot(df, aes(x=LCname, y=L8.ndvi))

p1 + transition_states(L8.savi) + ease_aes('sine-in-out')

p1 + transition_states(L8.savi, transition_length = 1, state_length = 1, wrap = FALSE) +
  shadow_mark() +
  enter_grow() +
  enter_fade()
...
p1_df <-  ggplot(df) + 
  geom_point(aes(x = LCname, y = L8.ndvi,color=SRTM)) 
p1_df


p1_df + transition_states(LUCAS_LC, transition_length = 1, state_length = 1, wrap = FALSE) +
  shadow_mark() +
  enter_grow() +
  enter_fade()


ggplot(df, aes(MOD.evi, MOD.ndvi, size = SRTM, colour = LCname)) +
  geom_point(alpha = 0.7, show.legend = FALSE) +
  scale_size(range = c(2, 12)) +
  scale_x_log10() +
  # Here comes the gganimate specific bits
  labs(x = 'MODIS EVI', y = 'MODIS NDVI') +
  transition_states(LCname) 


##Nice Example with gapminder - data
#Redo with your own data z.B. Quellen
