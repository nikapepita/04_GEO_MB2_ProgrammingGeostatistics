...
# Data subset to desired years e.g. 1961-2018
# 1: 1881
# 80: 1980
# 138: 2018
#Number and year
...
#month of interest
month <- as.character("August")

#TimeSeries
start_time <- as.character(1961)
end_time <- as.character(2000)

# year for comparison to long term statistics
year_comp <- 1979

time_ser <- paste ("Year",year_comp, sep = "_", collapse = NULL)

my_years <- c(seq(1881, 2019, by=1))
...
result_tidy <- result_tidy[c(seq(start[1],end[1], by=1))]
...
layer_names <- c(paste0("Year_", seq(start[2], end[2], by=1)))
...
rasterHist <- my_raster[[grep(start_time, layer_names):grep(end_time, layer_names)]]

# year for comparison to long term statistics
rasterComp <- my_raster[[grep(time_ser,layer_names)]]
...
# Calculate mean temperature between 1961 and 2019
...
p1 <- ggR(rasterHist_mean, geom_raster = T)+
  ...
  # , guide = F
  labs(x="",y="")+
  ggtitle(paste ("Mean Temperatures", month, start[2],"-", end[2], sep = " ", collapse = NULL))+
...
p2 <- ggR(rasterComp, geom_raster = T)+
  scale_fill_gradient2(low="blue", mid='yellow', high="red", name ="temperature", na.value = NA, limits=c(minVal,maxVal))+
  labs(x="",y="")+
  ggtitle(paste ("Temperature", month, time_ser, sep = " ", collapse = NULL))+
  theme(plot.title = element_text(hjust = 0.5, face="bold", size=15))+
  theme(legend.title = element_text(size = 12, face = "bold"))+
  theme(legend.text = element_text(size = 10))+
  theme(axis.text.y = element_text(angle=90))+
...
pdf(paste ("Temperature_mean", month, end[2], ".pdf",sep = "", collapse = NULL), width = 14, height = 8)
...
dfab <- rbind(data.frame(df,band=paste(start[2],"-", end[2], "(mean)",sep = " ", collapse = NULL)), data.frame(df2,band=as.character(end[2])))

pdf(paste ("Temperature_mean_v2", month, end[2], ".pdf", sep = "", collapse = NULL), width = 14, height = 8)
...
pdf(paste ("August_mean_vs", month, end[2], "_diff.pdf", sep = "", collapse = NULL), width = 14, height = 8)
