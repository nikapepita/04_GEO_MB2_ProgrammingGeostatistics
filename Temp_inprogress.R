...
#####Make your own idea out of it: Soil moisture - idea
#or did we have normal weather condition in garmisch?
################################
### Download from ftp server ###
################################

#load Shapefile Hikinig route and buffer it
track <- shapefile("Wanderroute.shp")
track_buffer <- gBuffer(track, width = 100, quadsegs = 10)
...
#http <- "ftp://opendata.dwd.de/climate_environment/CDC/grids_germany/monthly/soil_moist/"


http_temp <- "ftp://opendata.dwd.de/climate_environment/CDC/grids_germany/monthly/air_temperature_mean/12_Dec/"

#soil
http_soil_moist <- "ftp://opendata.dwd.de/climate_environment/CDC/grids_germany/monthly/soil_moist/"

#frost
http_frost_depth <- "ftp://opendata.dwd.de/climate_environment/CDC/grids_germany/monthly/frost_depth/"
...
library(RCurl)
result_temp <- getURL(http_temp, verbose=TRUE, ftp.use.epsv=TRUE, dirlistonly = TRUE)

# Split string into pieces by identifying certain pattern that seperates the individual filenames
library(tidyverse)

result_tidy <- str_split(result_temp, "\n|\r\n")  # sometimes \r needed #can spilt chr string, afterwards you can grep the URL
...
# 80: 1980
# 138: 2018
result_tidy <- result_tidy[c(seq(130,138, by=1))]
...
dir.create("DWDdata_Hiking1/")
out_dir <- "DWDdata_Hiking1/"
...
for (i in 1:length(result_tidy)) {
  ...
  {
    download.file(paste0(http_temp, result_tidy[i]), paste0(out_dir, result_tidy[i]))
  }
}
...
layer_names <- c(paste0("year_", seq(2010, 2018, by=1)))
names(my_raster) <- layer_names

# crop and mask the data

track_buffer <- spTransform(track_buffer,"+init=epsg:31467" )
my_raster <- crop(my_raster, track_buffer)
...
rasterHist <- my_raster[[grep("2010", layer_names):grep("2018", layer_names)]]
...
#based on this - the legend is for both plots the same
